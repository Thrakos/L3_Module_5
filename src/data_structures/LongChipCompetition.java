...
public class LongChipCompetition {
	...
	public static void main(String[] args) {
		...
		System.out.println(LCC.findLongestChip(theBeatles));
	}
	
	String findLongestChip(ArrayList<Beatle> b) {
		for (int i = 0; i < b.size(); i++) {
			for (int j = 0; j < b.get(i).getChips().size(); j++) {
				if (b.get(i).getChips().get(i).getLength() > longest) {
					longest = b.get(i).getChips().get(i).getLength();
					winner = b.get(i).getName();
				}
			}
		}
		return winner;
